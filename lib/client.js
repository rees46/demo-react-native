...
import { SESSION_CODE_EXPIRE, DEBUG } from "../index";
import DataEncoder from "./utils";

const encoder = new DataEncoder();
const SERVER_URL = "https://api.rees46.ru/";

const getData = async () => {
  try {
    return JSON.parse(await AsyncStorage.getItem("@rees46")) ?? {};
  } catch (error) {
    return error;
  }
};
...
export async function updSeance (did = '', seance = '') {
  ...
  try {
    ...
    await AsyncStorage.setItem('@rees46', JSON.stringify(data));
  } catch (error) {
...
export async function request(url, options = {}, contentType = "application/x-www-form-urlencoded") {
  const config = {
  ...
  const params = {
    ...
    params: contentType === 'application/x-www-form-urlencoded' ? options?.method === 'POST' ? config.params : encoder.convertToObject(encodeString) : '',
    data: contentType === 'application/x-www-form-urlencoded' ? config.payload : config.params,
  };
